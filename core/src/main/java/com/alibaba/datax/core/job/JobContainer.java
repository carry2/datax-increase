...
import com.alibaba.datax.core.util.PropertiesUtil;
import com.alibaba.datax.core.util.StringUtil;
    ...
    public void start() {
        ...
        try {
            ...
            if(isDryRun) {
                ...
                String increasecolumn = configuration.getString(CoreConstant.DATAX_JOB_CONTENT_READER_PARAMETER_INCREASECLOUMN);

                LOG.info("increasecolumn   value  is {}",increasecolumn);
                //增量同步改造  如果是增量同步 则修改配置文件时间
                if(org.apache.commons.lang3.StringUtils.isNotBlank(increasecolumn)){
                    //增量同步改造
                    String jdbcurl = configuration.getString(CoreConstant.DATAX_JOB_CONTENT_READER_PARAMETER_CONNECTION_JDBCURL);
                    String table = configuration.getString(CoreConstant.DATAX_JOB_CONTENT_READER_PARAMETER_CONNECTION_TABLE);
                    String[] split = jdbcurl.split("\\?");
                    String dataNameFromJdbcUrl = StringUtil.parseDataNameFromJdbcUrl(split[0]);
                    PropertiesUtil.upTime(dataNameFromJdbcUrl,table,format);
                    LOG.info("dataNameFromJdbcUrl   value  is {}",dataNameFromJdbcUrl);
                    LOG.info("table   value  is {}",table);
                    LOG.info("Date   value  is {}",format);
                }
                this.invokeHooks();
            }
        } catch (Throwable e) {
